...
class SpeechSynthesizer:
    ...
    def __init__(self, model: str = "auto", device: Optional[str] = None):
        ...
        try:
            ...
            for i in range(max_wait):
                ...
                if self.server_process.poll() is not None:
                    ...
                    logger.warning(f"OpenAudio server process died: {stderr.decode('utf-8', errors='ignore')}")
                    return False
                
                try:
                    response = requests.get(f"http://localhost:{self.api_port}/v1/health", timeout=1)
                    if response.status_code == 200:
                        logger.info(f"OpenAudio API server started successfully on port {self.api_port}")
                        self.api_running = True
                        return True
                except:
                    pass
